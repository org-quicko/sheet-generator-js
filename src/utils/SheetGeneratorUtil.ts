...
import { ListGeneratorUtil } from './ListGeneratorUtil.js';
import { SchemaRefResolverUtil } from './SchemaRefResolverUtil.js';
import { TableGeneratorUtil } from './TableGeneratorUtil.js';
...
export class SheetGeneratorUtil {
    public static generate(
        ...
        tablesDir: string,
        listsDir: string,
        classNames: { sheet: string[], table: string[], list: string[], row: string[] }
    ): void {
        ...
        blockOneOf.forEach((block: JsonSchema) => {
            ...
            const importPath = `../../${importDir}${GeneratorUtil.toKebabCase(blockNameEnum)}/${blockClassName}.js`;
    ...
    constructor() {
        ...
        ${blockOneOf
            .map((blockSchema: JsonSchema) => {
                ...
                const bClassName = GeneratorUtil.sanitizeName(
                    GeneratorUtil.capitalize(GeneratorUtil.toCamelCase(bName)),
                    bEntity === 'list' ? 'List' : 'Table'
                );
